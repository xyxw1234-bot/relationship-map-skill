...
def run(contacts:int, rounds:int):
    ...
    try:
        ...
        assert_true(len(view['rows'])<=15,'文本列表未分页')
        assert_true(all(not e['leaked'] for e in view['rows']),'一级文本列表泄露敏感字段')
        assert_true('想看某个人的详情' in view['text'],'列表缺少自然语言下一步')
        forbidden=['['+'看某个人详情'+']','历史控件','inter'+'active','ca'+'rd','*'+'*','#'+'#','`'+'``']
        ...
        state2={'page':1,'page_size':15,'query':'','city':'重庆','sort':'updated_desc'}; c2=adapter.list_text(state2)
        ...
        for i in range(rounds):
            ...
            view=adapter.list_text({'page':page,'page_size':15,'query':'','city':city,'sort':'updated_desc'})
            assert_true(len(view['rows'])<=15,'压力分页失败')
